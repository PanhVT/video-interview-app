...
export default function InterviewPage() {
  ...
  const initialFlags = useMemo(() => Array(questions.length).fill(false), [questions.length])
  ...
  useEffect(() => {
    ...
    async function loadStream() {
      try {
        streamHandle = await navigator.mediaDevices.getUserMedia({ video: true, audio: true })
  ...
  const updateCompletedFlag = useCallback((value) => {
  ...
  const formattedTime = `${String(Math.floor(timeLeft / 60)).padStart(1, '0')}:${String(timeLeft % 60).padStart(2, '0')}`
    ...
    return (
      <div className="interview-start-screen theme-transition">
        ...
        <div className="start-card theme-transition">
          <h2>Interview</h2>
          <p>Make sure you're ready. The interview begins when you start recording.</p>
    ...
    <div className="interview-screen theme-transition">
      ...
      <div className="interview-frame">
        ...
        <div className="question-row">
          <div className="question-strip theme-transition">
            {questions.map((_, i) => (
              <div
                key={i}
                className={`question-pill ${i === index ? 'active' : ''} ${completedFlags[i] ? 'done' : ''}`}
              >
                {i + 1}
              </div>
            ))}
        ...
        <div className="video-stage theme-transition">
          ...
          <div className={`timer-pill ${timeLeft <= 30 && status === 'recording' ? 'warning' : ''}`}>
            {formattedTime}
          </div>
